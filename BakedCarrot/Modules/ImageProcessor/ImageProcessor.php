<?
	require 'ImageProcessorException.php';
	...
	class ImageProcessor
	{
		...
		private $format_supported = array();
		private $dir_mask = 0777;
		private $file_mask = 0665;
		private $width;
		private $height;
		private $type;
		private $mime;
		...
		{
			if(!$info = @gd_info()) {
				throw new ImageProcessorException('GD extension not installed');
			}
		...
		public function loadFromFile($file)
		{
			if(!is_readable($file) || is_dir($file)) {
				throw new ImageProcessorException('File is unreadable: ' . $file);
			}
			
			$this->image_info = @getimagesize($file);
			
			if(!$this->image_info) {
				throw new ImageProcessorException('Invalid file format');
			}
			
			if(!$this->formatSupported($this->image_info[2])) {
				throw new ImageProcessorException('File format not supported');
			}
		...
		public function loadFromUrl($url)
		{
			if(strpos($url, 'http://') !== 0 && strpos($url, 'https://') !== 0) {
				throw new ImageProcessorException('Wrong url format');
			}
		
			if(!($result = @file_get_contents($url))) {
				throw new ImageProcessorException('Cannot load image or file format not supported');
			}
			
			if(($this->handle = imagecreatefromstring($result)) === false) {
				throw new ImageProcessorException('Cannot create image from url');
			}
		...
		public function upload($file, $dest, $remove_orig = true)
		{
			...
			if(!is_dir($orig_file_info['dirname'])) {
				if(!mkdir($orig_file_info['dirname'], $this->dir_mask, true)) {
					throw new ImageProcessorException('Cannot create directory: ' . $orig_file_info['dirname']);
				}
			}
			...
			if(is_uploaded_file($file)) {
				if(!move_uploaded_file($file, $dest)) {
					throw new ImageProcessorException('File upload error: ' . $file);
				}
			}
			else {
				if($remove_orig && !rename($file, $dest)) {
					throw new ImageProcessorException('Cannot move file ' . $file . ' to ' . $dest);
				}
				
				if(!$remove_orig && !copy($file, $dest)) {
					throw new ImageProcessorException('Cannot copy file ' . $file . ' to ' . $dest);
				}
		...
		public function resize($width, $height, $base = self::RESIZE_BASE_MAX, $resize_if_smaller = false)
		{
			...
			if(!$coef) {
				throw new ImageProcessorException('Invalid "base" parameter');
			}
			...
			if(!imagecopyresampled($im_new, $this->handle, 0, 0, 0, 0, $new_width, $new_height, $this->width, $this->height)) {
				throw new ImageProcessorException('Image resizing failed');
			}
		...
		public function crop($width, $height, $base = self::CROP_BASE_TOPLEFT)
		{
			...
			if(!$ret) {
				throw new ImageProcessorException('Image cropping failed');
			}
		...
		public function sharpen($amount)
		{
			...
			if(!imageconvolution($this->handle, $matrix, $div, 0)) {
				throw new ImageProcessorException('Image sharpen failed');
			}
		...
		public function filter()
		{
			...
			if(!call_user_func_array('imagefilter', $arg_list)) {
				throw new ImageProcessorException('Cannot apply filter');
			}
		...
		{	
			...
			return $this->resize($width, $width, self::RESIZE_BASE_MAX)
				->crop($width, $width, ImageProcessor::CROP_BASE_CENTER)
				->sharpen(20)
				->saveAs($file, $quality);
		}
		...
		public function saveAs($file, $quality = 90)
		{
			...
			if(!is_dir($file_info['dirname'])) {
				if(!mkdir($file_info['dirname'], $this->dir_mask, true)) {
					throw new ImageProcessorException('Cannot create directory: ' . $file_info['dirname']);
				}
			}
			...
			if($res == false) {
				throw new ImageProcessorException('Cannot save file ' . $file);
			}
		...
		private function loadData()
		{
			...
			if(is_null($this->handle)) {
				throw new ImageProcessorException('Cannot load image data from file ' . $this->image_file);
			}
		}
		...
		{
			if(is_null($this->handle)) {
				throw new ImageProcessorException('No image loaded');
			}
		}
