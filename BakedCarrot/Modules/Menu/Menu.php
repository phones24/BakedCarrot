<?
	...
	class Menu
	{
		...
		public function __construct($params = null)
		{
			...
			$this->ignore_prefix = isset($params['ignore_prefix']) ? $params['ignore_prefix'] : null;
			
			$driver_class = $params['driver'];
		...
		public function getSelectedItem()
		{
			if(!$this->selected_item) {
				$this->selected_item = $this->findItem('selected', true);
			}
		...
		public function findItem($key, $value, $item = null)
		{
			if(is_null($item)) {
			...
			if($item->hasChildren()) {
				foreach($item->getChildren() as $child) {
					if($item = $this->findItem($key, $value, $child)) {
						return $item;
					}
				}
			}
		...
		public function getFlatArray($min_depth = 0, $max_depth = -1, $cond = null)
		{
			...
			if(is_array($cond) && !empty($cond)) {
				$key = reset($cond);
				$start = $this->findItem($key, $cond[$key]);
			}
			else {
				$start = $this->getRootItem();
			}
